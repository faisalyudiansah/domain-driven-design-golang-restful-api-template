...
type UserRepository interface {
	...
	SaveUserDetail(ctx context.Context, userId int64, fullname string) (*entity.UserDetail, error)
...
func (r *userRepositoryImpl) SaveUserDetail(ctx context.Context, userId int64, fullname string) (*entity.UserDetail, error) {
	query := `
		INSERT INTO user_details (user_id, full_name, whatsapp_number, image_url, created_at, updated_at) VALUES 
		($1, $2, NULL, $3, NOW(), NOW())
		RETURNING id, user_id, full_name, whatsapp_number, image_url, created_at, updated_at, deleted_at;
	`
	...
	tx := transactor.ExtractTx(ctx)
	if tx != nil {
		err = tx.QueryRowContext(ctx, query, userId, fullname, defaultImgUrl).Scan(
			&userDetail.ID,
			&userDetail.UserId,
			&userDetail.Fullname,
		...
		err = r.db.QueryRowContext(ctx, query, userId, fullname, defaultImgUrl).Scan(
			&userDetail.ID,
			&userDetail.UserId,
			&userDetail.Fullname,
			&userDetail.WhatsappNumber,
